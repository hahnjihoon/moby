...
class HtmlZktlxmScreen extends StatefulWidget {
  final List<Zktlxm> data;

  HtmlZktlxmScreen({Key? key, required this.data}) : super(key: key);

  @override
  _HtmlZktlxmScreenState createState() => _HtmlZktlxmScreenState();
}

class _HtmlZktlxmScreenState extends State<HtmlZktlxmScreen> {
  List<Zktlxm> filteredData = [];
  ...
  List<Zktlxm> replaceEmptyValues(List<Zktlxm> data) {
    List<Zktlxm> updatedData = [];
    for (var product in data) {
      // 각각의 Product 객체를 새로운 객체로 생성하여 처리합니다.
      // 그래야 원본 데이터를 변경하지 않고 새로운 데이터를 만들 수 있습니다.
      Zktlxm updatedProduct = Zktlxm(
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
          title: Text('moby_zktlxm_table_compare'),
