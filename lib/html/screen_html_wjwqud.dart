...
class HtmlWjwqudScreen extends StatefulWidget {
  final List<Wjwqud> data;

  HtmlWjwqudScreen({Key? key, required this.data}) : super(key: key);

  @override
  _HtmlWjwqudScreenState createState() => _HtmlWjwqudScreenState();
}

class _HtmlWjwqudScreenState extends State<HtmlWjwqudScreen> {
  List<Wjwqud> filteredData = [];
  ...
  List<Wjwqud> replaceEmptyValues(List<Wjwqud> data) {
    List<Wjwqud> updatedData = [];
    for (var product in data) {
      // 각각의 Product 객체를 새로운 객체로 생성하여 처리합니다.
      // 그래야 원본 데이터를 변경하지 않고 새로운 데이터를 만들 수 있습니다.
      Wjwqud updatedProduct = Wjwqud(
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
          title: Text('moby_wjwqud_table_compare'),
