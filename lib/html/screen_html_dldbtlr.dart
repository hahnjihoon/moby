...
class HtmlDldbtlrScreen extends StatefulWidget {
  final List<Dldbtlr> data;

  HtmlDldbtlrScreen({Key? key, required this.data}) : super(key: key);

  @override
  _HtmlDldbtlrScreenState createState() => _HtmlDldbtlrScreenState();
}

class _HtmlDldbtlrScreenState extends State<HtmlDldbtlrScreen> {
  List<Dldbtlr> filteredData = [];
  ...
  List<Dldbtlr> replaceEmptyValues(List<Dldbtlr> data) {
    List<Dldbtlr> updatedData = [];
    for (var product in data) {
      // 각각의 Product 객체를 새로운 객체로 생성하여 처리합니다.
      // 그래야 원본 데이터를 변경하지 않고 새로운 데이터를 만들 수 있습니다.
      Dldbtlr updatedProduct = Dldbtlr(
